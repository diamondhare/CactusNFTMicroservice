...
export type TransferSeedTokenInput = {
  tokenId: bigint;
...
@Injectable()
export class SeedTransferStrategy {
  constructor(
  ...
  async transferSeedToken(input: TransferSeedTokenInput): Promise<string> {
    ...
    const toWallet = this.botWalletsService.getWallet(input.toWalletIndex);
    const seed721 = new Contract(
      seed721Address,
      SEED_721_ABI,
      this.provider,
    ).connect(fromWallet);
    const transferFrom = seed721.getFunction('transferFrom');
