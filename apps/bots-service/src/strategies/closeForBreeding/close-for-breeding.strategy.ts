...
export type CloseForBreedingInput = {
  tokenId: bigint;
  fromWalletIndex: number;
};

@Injectable()
export class CloseForBreedingStrategy {
  constructor(
  ...
  async closeForBreeding(input: CloseForBreedingInput): Promise<string> {
    ...
    const closeForBreeding = seed721.getFunction('closeForBreeding');

    const tx = (await closeForBreeding(
      input.tokenId,
